...
{
public:
	DirectionalLight(const float3& color, const float3& dir, float intensity, const int specPow);
	...
	float3 calculateLightNoClamps(const float3 position, const float3 & normal, const VertexProcessor & vp) const override;
